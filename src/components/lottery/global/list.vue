  ...
  <div class="wheel-container" :style="{
    ...
    >
      <div class="wheel-item" v-for="(item, index) in prizeList" :key="index">
        ...
        >
          <img :src="item.images" style="width: 100%;height:auto;">
        </div>
      </div>
    </div>
    <div class="wheel-btn" @click="start">
...
export default {
  ...
  props: {
    ...
    bgImg: {
      type: String,
      default: 'https://gw.alipayobjects.com/zos/rmsportal/YIunNQVWkFRxUTaUNhOZ.png'
    },
    btnImg: {
      type: String,
      default: 'https://gw.alipayobjects.com/zos/rmsportal/JHenAywYHZTLbbrnkIFN.png'
    },
    onStart: {
      type: Function,
      default: () => {}
  ...
  methods: {
    getIndexByName (name) {
      ...
      for (let i = 0; i < this.count; i++) {
        ...
        if (list[i] && list[i].uuid === name) return i
      }
      return -1
    },
    start () {
