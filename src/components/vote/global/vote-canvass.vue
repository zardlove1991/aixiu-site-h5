<template>
  <div class="canvass-dialog-wrap" v-if="show" @click.stop="close()">
    ...
    <!-- <div class="poster-tips tips" v-if="isTipsShow && sharePoster">
      提示：长按图片保存或转发朋友圈
    </div> -->
    <notice :message="message" v-if="isTipsShow && sharePoster"></notice>
  ...
  methods: {
    ...
    shareMake(worksId, code) {
      ...
      API.getVoteWorksDetail({
        ...
        let coverExt =
          "?x-oss-process=image/resize,m_fixed,w_560,h_350,color_EAD5BA";
        ...
        if (res.material) {
          ...
          // 判断是否长图 如果是就修改为不截断参数
          if (isLongCover)
            params.cover = params.cover.replace("m_fixed", "m_pad");
        }
