    ...
    <form>
      ...
      <div v-if="showModel === 'picture'" class="form-item">
        ...
        <div class="form-tips-div" v-else-if="imageRatio && imageUpload_min_num">建议比例：4:5.6（1寸照片的比例尺寸,小于10M；图片最少上传{{imageUpload_min_num}}张,最多上传{{imageUpload_max_num}}张；支持PNG、JPG、GIF格式</div>
         <div class="form-tips-div" v-else-if="!imageRatio && imageUpload_min_num">小于10M;图片最少上传{{imageUpload_min_num}}张,最多上传{{imageUpload_max_num}}张；支持PNG、JPG、GIF格式</div>
        <div class="form-tips-div" v-else>小于10M；图片最多上传9张；支持PNG、JPG、GIF格式</div>
        <div class="form-content">
          <file-upload
