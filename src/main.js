...
router.beforeEach((to, from, next) => {
  let name = to.name
  let allArr = ['depencestart', 'votebegin', 'votedetail', 'enrollstart', 'livestart', 'newstart', 'drawlist', 'depencelist', 'VoteRank', 'newstopic']
...
router.afterEach((route, from) => {
  ...
  let isFirst = false
  let allArr = ['depencestart', 'votebegin', 'votedetail', 'enrollstart', 'livestart', 'newstart', 'drawlist', 'newstopic']
  if (allArr.includes(name) && !from.name) {
    isFirst = true
  }
